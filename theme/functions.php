...
/**
* Get title for the webpage by concatenating page specific title with side-wide title.
*
...
function get_title($title) {
	global $miletus;
	return $title . (isset($miletus['title_append']) ? $miletus['title_append'] : null);
...
function get_navbar($menu)
{
    ...
    $createMenu = function ($items, $callback) use (&$createMenu, $createUrl) {
        ...
        foreach ($items as $item) {
            ...
            $html .= "\n<li {$class}><a href='{$url}' title='{$item['title']}' {$class}>{$item['text']}</a>{$submenu}</li>\n";
