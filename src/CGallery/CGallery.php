<?php
/**
 * A class that display image files
 */

Class CGallery {
    private $path               = null;
    private $GALLERY_PATH       = null;
    private $GALLERY_BASEURL    = null;
     ...
     *
     */
    public function getGallery($path, $breadcrumb=true) {
        $this->path = realpath($this->GALLERY_PATH . DIRECTORY_SEPARATOR . $path);

        // Validate
        substr_compare($this->GALLERY_PATH, $this->path, 0, strlen($this->GALLERY_PATH)) == 0 or errorMessage('Security constraint: Source gallery is not directly below the directory GALLERY_PATH.');
        ...
        if($breadcrumb){$breadcrumdHTML = $this->createBreadcrumb();}

        $html = $breadcrumdHTML . $gallery;
